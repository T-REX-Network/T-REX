...
contract Create3Wrapper {
    ...
    /// Mirrors TREXFactory._saltBytes so we can assert the per-contractType discriminator never collides.
    function saltBytes(address factory, string memory salt, string memory contractType)
        external
        pure
        returns (bytes32)
    {
        return bytes20(factory) | keccak256(bytes(string.concat(salt, contractType))) >> 168;
    }
