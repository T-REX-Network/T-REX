 ...
 * itself is real state inside the EnumerableSet. `moduleTypes` is left NONDET, so a module may land in any
 * of the three type lists and no dispatch point is skipped: these rules are about access control, not
 * about type routing, which the Foundry dispatch suite covers.
 *
...
definition isRestricted(method f) returns bool =
    ...
    || f.selector == sig:setIssuancePaused(bytes32,bool).selector
    || f.selector == sig:discardExpiredValidations(uint256[]).selector;
