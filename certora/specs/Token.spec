 ...
 *    which matches the suite's `executionDelay == 0` deployment model.
 *  - This branch removed the ERC-2771 meta-tx surface, so `_msgSender() == msg.sender` unconditionally and
 *    there is no trusted-forwarder indirection to model.
 *
...
///   The wired ModularCompliance / IdentityRegistry are genuinely *external* contracts (not the token
