...
///         and exposes the `restricted`-gated `_msgSender()` used by the access-control rules.
///
...
///         high-level `IAuthority.canCall` call that the spec's `_.canCall` summary can intercept. No
...
contract TokenHarness is Token {
    ...
    /// @return the ERC-2771 trusted forwarder. The rules `require` this to be 0 so the meta-tx indirection is
    ///         disabled and `_msgSender() == msg.sender` — none of the Token properties concern meta-tx, and
    ///         `Token._msgSender()` is declared non-virtual (final), so it cannot be overridden in the harness.
    function trustedForwarderAddress() external view returns (address) {
