...
contract SuiteAuthorityMigrator {
    ...
    function _migrate(
        ...
        require(newAuthority.code.length != 0, ErrorsLib.NewAuthorityNotAContract(newAuthority));
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (rotateIdentities) {
                _addManagementKey(oldAuthority, identity, newAuthority);
            } else {
                require(
                    identity == address(0) || !_isManager(identity, address(oldAuthority)),
                    ErrorsLib.IdentityRotationRequired(identity)
                );
            }
        }
